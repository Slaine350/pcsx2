...
void Panels::MemoryCardListPanel_Simple::AppStatusEvent_OnSettingsApplied()
{
	for (uint slot = 0; slot < 8; ++slot)
	{
		...
		wxString targetFile = Path::ToWxString(Path::FromWxString(GetMcdPath().ToString()) / m_Cards[slot].Filename);
...
void Panels::MemoryCardListPanel_Simple::UiDeleteCard(McdSlotItem& card)
{
	...
	if (result)
	{
		wxFileName fullpath(Path::ToWxString(Path::FromWxString(m_FolderPicker->GetPath().ToString()) / card.Filename));
...
bool Panels::MemoryCardListPanel_Simple::UiDuplicateCard(McdSlotItem& src, McdSlotItem& dest)
{
	...
	while (1)
	{
		...
		wxFileName srcfile(Path::ToWxString(Path::FromWxString(m_FolderPicker->GetPath().ToString()) / src.Filename));
		wxFileName destfile(Path::ToWxString(Path::FromWxString(m_FolderPicker->GetPath().ToString()) / dest.Filename));
...
void Panels::MemoryCardListPanel_Simple::UiRenameCard(McdSlotItem& card)
{
	...
	if (!wxRenameFile(Path::ToWxString(Path::FromWxString(basepath.ToString()) / card.Filename), (basepath + wxFileName(newFilename)).GetFullPath(), false))
	{
