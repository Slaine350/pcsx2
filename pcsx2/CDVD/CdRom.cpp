...
void interpolate(xa_decode_t buffer)
{
	...
	s16* channelLeft = buffer.pcm;
	s16* channelRight;

	for (int i = 0; i < 7; i++)
	{
		zigZagInterpolation(channelLeft, table[i], p);
		if (cdr.Xa.stereo)
		{
			zigZagInterpolation(channelRight, table[i], p);
		}
	}
}
...
s16 zigZagInterpolation(s16* ringbuff, s16* tableRow, int p)
{
	...
	{
		sample += (s32)ringbuff[(p - i) & 0x1F] * tableRow[i] / 0x8000;
	}
